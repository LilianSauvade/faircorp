...
public class SpringSecurityConfig {
    ...
    public UserDetailsService userDetailsService() {
        ...
        manager.createUser(
                User.withUsername("user").password(encoder.encode("myUser")).roles(ROLE_USER).build()
        );
        manager.createUser(
                User.withUsername("admin").password(encoder.encode("myAdmin")).roles(ROLE_ADMIN).build()
        );
        return manager;
    }
